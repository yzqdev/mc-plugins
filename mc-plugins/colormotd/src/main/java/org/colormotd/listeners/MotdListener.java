 ...
 */
package org.colormotd.listeners;
...
public class MotdListener implements PacketListener {
    ...
    public MotdListener(ColorMotd plugin) {
        this.plugin = plugin;
    ...
    public void onPacketSending(PacketEvent event) {
        ...
        String motd = config.isMaintenanceMode() ? plugin.getHookPlaceholder().applyPlaceHolder(config.getMaintenanceModeMotd(),ip) : config.randomMotd();
        BufferedImage favicon = plugin.favicons().chooseFavicon(config.isMaintenanceMode());

        status.setMotD(WrappedChatComponent.fromText(plugin.getHookPlaceholder().applyPlaceHolder(motd, ip)));
        status.setFavicon(favicon == null ? null : faviconCache.getUnchecked(favicon));

        if (config.isShowPing()) {
            String onlineMsg = plugin.getHookPlaceholder().applyPlaceHolder(config.randomOnlineMsg(),ip);
            if (onlineMsg != null) {
        ...
        if (players != null && !players.isEmpty()) {
            status.setPlayers(config.getPlayers().stream()
                    .map(line -> plugin.getHookPlaceholder().applyPlaceHolder(line,ip))
