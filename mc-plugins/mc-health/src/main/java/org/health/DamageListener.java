...
import org.health.*;
import org.health.utils.MobBarsUtils;
...
public class DamageListener implements Listener {
    ...
    public void onEntityRegain(EntityRegainHealthEvent event) {
        ...
        if (playerEnabled) {
            if (entity instanceof Player) {
                parsePlayerHit((Player) entity, event.getRegainReason() != EntityRegainHealthEvent.RegainReason.SATIATED && event.getAmount() > 0.0);
                return;
            }
        }
        if (mobEnabled) {
