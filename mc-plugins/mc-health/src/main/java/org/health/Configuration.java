...
public class Configuration {
    public enum Nodes {
        PLAYERS_ENABLE("player-bars.enable", true), PLAYERS_AFTER_ENABLE("player-bars.after-name.enable", true),
        ...
        MOB_ENABLE("mob-bars.enable", true), MOB_SHOW_ON_NAMED("mob-bars.show-on-named-mobs", true),
        MOB_STYLE("mob-bars.display-style", 1), MOB_ALWAYS_SHOWN("mob-bars.always-shown", false),
        MOB_TEXT_MODE("mob-bars.text-mode", false), MOB_CUSTOM_TEXT_ENABLE("mob-bars.custom-text-enable", false),
        MOB_CUSTOM_TEXT("mob-bars.custom-text", "{name} - &a{health}/{max}"),
        MOB_DELAY("mob-bars.hide-delay-seconds", 5), MOB_SHOW_IF_LOOKING("mob-bars.show-only-if-looking", false),
        MOB_USE_CUSTOM("mob-bars.use-custom-file", false), MOB_WORLD_DISABLING("mob-bars.world-disabling", false),
        ...
        HOOKS_EPIBOSS("hooks.epicboss", false), FIX_TAB_NAMES("fix-tab-names", true),
        FIX_DEATH_MESSAGES("fix-death-messages", true), UPDATE_NOTIFICATION("update-notification", true),
